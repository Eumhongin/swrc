<?php
	...
	if($idx > 0){
		...
		if($menuArr){
			...
			$UpperMenuArr = getUpperMenuRow($parentIdx, $row[code]);
			$LowerMenuArr = getLowerMenuRow($parentIdx, $row[code]);
...
<!--
	...
	function moveDownMenu(idx){
		var processUrl = parent.frmPnt.PROCESS_URL.value; 
		parent.document.location.href  = processUrl+"&page=/pages/admin/menu/menuSortEdit.php&amp;idx="+idx+"&amp;parentIdx=<?= $parentIdx?>&amp;action=down";
	}
	function moveUpMenu(idx){
		var processUrl = parent.frmPnt.PROCESS_URL.value; 
		parent.document.location.href  = processUrl+"&page=/pages/admin/menu/menuSortEdit.php&amp;idx="+idx+"&amp;parentIdx=<?=$parentIdx?>&amp;action=up";
	}
		...
		<tbody>	
			...
			<tr>
				...
				<td class="tal">
					...
					이름 :<input type="text" name="chargeUserName" id="chargeUserName" class="basic"  style="width:100px;" value="<?=$charge_user_name?>"/>&nbsp;
					전화번호 :<input type="text" name="chargeUserTel" id="chargeUserTel" class="basic"  style="width:100px;" value="<?=$charge_user_tel?>"/>
			...
			<tr>
			<th><label for="matchKeyword">연결 키워드</label></th>
			<td colspan="3" class="tal">
				
						<select id="matchKeywords" name="matchKeywords" onchange="oneChoice(this);" style="width:200px; font-size:9pt;" size="3" multiple>
<?php
	if ($menuMatchKeywordList){
		while($row = mysql_fatch_array($menuMatchKeywordList)){
?>
...
?>
						...
						<a href="javascript:deleteMatchKeyword();" class="btn_bgBlue">
							<img src="/pages/admin/images/bbs/btn_select_del.gif" alt="선택삭제" class="vmiddle" />
						</a>
						
						<br/><br/>
											
						<input type="text" id="matchKeyword" name="matchKeyword" value="" class="basic" style="width:200px;" maxlength="50" />
						<a href="javascript:addMatchKeyword();" class="btn_bgBlue">
							<img src="/pages/admin/images/bbs/btn_add.gif" alt="추가" class="vmiddle" />
						</a>
...
?>
						...
						<td>
							<a href="/open_content/sub.php?menuIdx=<?= $idx ?>&amp;page=/contents/pages/<?=$row[idx] ?>.php" target="_blank" class="btn_bgBlue" title="<?=$row[title] ?> 콘텐츠 미리보기">
