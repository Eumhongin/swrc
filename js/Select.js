...
SS_ENV.CR.ReverseBackground = '#DBDBDB';
SS_ENV.CR.ReverseText = '#4B4B4B';
SS_ENV.CR.Border = '#A6A6A6';
SS_ENV.CR.BorderActive = '#797979';
SS_ENV.ImgPrefix = 'http://imgnews.naver.com/image/image';
SS_ENV.DefaultHeight = 19;
SS_ENV.ButtonWidth = 20;
...
function SS_createTitleDiv ()
{
  ...
  this.TitleTable.cells(0).childNodes(0).innerText = this.options[this.selectedIndex].innerText;
  this.TitleTable.cells(1).innerHTML = "<img src='http://static.naver.com/news/images2/buttn_down00.gif' border=0 align=absmiddle>";
  this.TitleTable.cells(2).innerHTML = "<img src='http://static.naver.com/news/images2/buttn_down11.gif' border=0 align=absmiddle>";
...
function NewWindow(url) {
    if (url) {
        window.open( url, 'VOD', 'toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=yes,scrollbars=no,width=710,height=500,top=30,left=230');
    }
}

var enableFlag = 0;
function enableKeywords() {
	var list = document.getElementsByName("keyword");
	var length = list.length;
	for ( var i = 0; i < length; i++) {
		list[i].style.textDecorationUnderline = 'true';
		list[i].style.color = 'blue';
		list[i].style.cursor = 'hand';
	}
	enableFlag = 1;
}
function disableKeywords() {
	...
	var length = list.length;
	for ( var i = 0; i < length; i++) {
		list[i].style.textDecorationNone = 'true';
		list[i].style.color = 'black';
		list[i].style.cursor = 'none';
	}
	enableFlag = 0;
}
function OnClick(link, target) {
    var strlow = link;
    var nPos = -1;
	...
	target = "_blank";
	link = "http://stock.naver.com/sise/ksc_main.php?code="+link+"&ref=news_read";
